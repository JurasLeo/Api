...
const drzave = document.getElementById('drzave');
const urlDrzave = 'https://sudreg-data.gov.hr/api/javni/sudovi';

const drzaveContainer = document.getElementById('drzave');
...
/////ispis odabranih
const urlTvrtke = 'https://sudreg-data.gov.hr/api/javni/tvrtke';
...
async function searchCompany() {
    ...
    try {
        const response = await fetch(`${urlTvrtke}?mbs=${mbsInput}`, {
            headers: {
                'Authorization': `Bearer ${accessToken}`
            }
        });
